...
type Match = {
    ...
    rating?: number;         // ⬅️ pridaj
    plus_minus?: number;     // ⬅️ pridaj
};
...
export default function MatchesScreen() {
    ...
    const [voteLockDays, setVoteLockDays] = useState(0); // default 0 dni

    const fetchMatches = async () => {
        try {
            const response = await fetchWithAuth(`${BASE_URL}/matches/`);
            if (response.ok) {
                const data = await response.json();
                setMatches(data.matches);           // ⬅️ zápasy
                setVoteLockDays(data.vote_lock_days); // ⬅️ nastavenie z backendu
            } else {
    ...
    const updateStatus = async (matchId: number, status: "confirmed" | "declined") => {
        ...
        if (diffDays < voteLockDays) {
            Alert.alert("Zmenu stavu je možné vykonať najneskôr {lock_days} dni pred zápasom.");
            return;
        }
                        ...
                        return (
                            <TouchableOpacity key={m.id} onPress={() => router.push(`/match/${m.id}`)}>
                                ...
                                >
                                    ...
                                    {m.plus_minus != null && (
                                        <Text style={styles.stats}>± {m.plus_minus >= 0 ? "+" : ""}{m.plus_minus}</Text>
                                    )}
                                    ...
                                    {editable ? (
                                        <View style={{ flexDirection: "row", marginTop: 10, gap: 10 }}>
                                            {["confirmed", "declined"].map((status) => {
                                                ...
                                                return (
                                                    <TouchableOpacity
                                                        key={status}
                                                        onPress={() => updateStatus(m.id, status as any)}
                                                        style={{
                                                            backgroundColor,
                                                            paddingVertical: 6,
                                        ...
                                        <Text style={styles.note}>
                                            Zmena účasti už nie je možná (menej ako 2 dni pred zápasom)
                                        </Text>
                                    )}
                                </ImageBackground>
                            </TouchableOpacity>
